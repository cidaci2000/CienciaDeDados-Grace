...
async function vizualizarInformacoesGlobais() {
    ...
    const pessoasComCasaPropria = (dados.total_pessoas_com_casa_propria / 1e9)
    ...
    paragrafo.innerHTML = `Você sabia que o mundo tem <span>${pessoasNoMundo} bilhões</span> de pessoas, e que aproximadamente <span>${pessoasComCasaPropria} bilhões</span> possuem casa própria, passando, em média, <span>${horas} horas</span> e <span>${minutos} minutos</span> em casa? <br>Isso significa que aproximadamente <span>${porcentagemPessoasComCasaPropria.toFixed(2)}%</span> das pessoas possuem uma casa própria e que passam <span>${porcentagemTempoEmCasa}%</span> do seu tempo diário nela.`
